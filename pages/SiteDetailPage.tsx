...
const SiteDetailPage: React.FC = () => {
  ...
  const { theme, healthStatus } = useContext(AppContext)!;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const telemetry = await fetchTimeseriesData('last_6h');
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard title="Inverter Health" value={healthStatus?.inverter_health ?? 0} unit="%" icon={<CircuitBoard className="w-6 h-6 text-violet-500" />} isLoading={isLoading} />
